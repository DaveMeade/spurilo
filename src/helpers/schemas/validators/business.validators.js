...
const participantLimitValidator = {
    validator: async function(participants) {
        ...
        const maxParticipants = await ConfigManager.get('systems.audit.maxParticipants', 50);
        return participants.length <= maxParticipants;
    },
    message: async () => {
        const max = await ConfigManager.get('systems.audit.maxParticipants', 50);
...
const controlOwnerAssignmentValidator = {
    validator: async function(controlOwnerId) {
        ...
        const Engagement = require('mongoose').model('Engagement');
        const engagement = await Engagement.findOne({ id: this.engagement_id });
        
        if (!engagement) return false;
        
        // Check if control owner is a participant with controlOwner role
        return engagement.participants.some(p => 
            p.user_id === controlOwnerId && 
            p.roles.includes('controlOwner')
        );
    },
...
const userOrganizationDomainValidator = {
    validator: async function(email) {
        ...
        const org = await Organization.findOne({ id: this.organizationId });
        
        if (!org || !org.org_domains || org.org_domains.length === 0) {
            return true; // No domain restrictions
        }
        
        const userDomain = email.split('@')[1];
        return org.org_domains.includes(userDomain);
    },
